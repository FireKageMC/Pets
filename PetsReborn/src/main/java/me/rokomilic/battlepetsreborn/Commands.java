    ...
    public static boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("reload")) {
            if (sender.hasPermission("battlepets.reload")) {
                BattlePetsReborn.plugin.reloadConfig();
                ((BattlePetsReborn) BattlePetsReborn.plugin).reload();
        ...
        if (args[0].equalsIgnoreCase("set") && sender instanceof Player) {
            ...
            if (!BattlePetsReborn.pets.containsKey(((Player) sender).getUniqueId())) {
                sender.sendMessage(Language.getMessage("NoPetAlive"));
                return true;
            }
            LivingEntity pet = BattlePetsReborn.pets.get(((Player) sender).getUniqueId());
            double kiekis = 0;
            try {
            ...
            switch (args[1]) {
                case "skillpoints":
                    pet.setMetadata("Points", new FixedMetadataValue(BattlePetsReborn.plugin, (int) kiekis));
                    break;
                case "xp":
                    pet.setMetadata("XP", new FixedMetadataValue(BattlePetsReborn.plugin,
                ...
                case "level":
                    ...
                    if (curr < neww) {
                        ...
                        if (st.equalsIgnoreCase("endermite"))
                            st = "block";
                        MobStats stats = BattlePetsReborn.statsai.get(st);
                        pet.setMetadata("Points", new FixedMetadataValue(BattlePetsReborn.plugin, pet.getMetadata("Points").get(0).asInt() + stats.SkillpointsForLevel * (neww - curr)));
                    }
                    pet.setMetadata("Level", new FixedMetadataValue(BattlePetsReborn.plugin, (int) kiekis));
                    BattlePetsReborn.spawning.update(pet, (BattlePetsReborn) BattlePetsReborn.plugin);
                    ...
                    break;
                case "vitality":
                    pet.setMetadata("Vitality", new FixedMetadataValue(BattlePetsReborn.plugin, (int) kiekis));
                    BattlePetsReborn.spawning.update(pet, (BattlePetsReborn) BattlePetsReborn.plugin);
                    break;
                case "dexterity":
                    pet.setMetadata("Dexterity", new FixedMetadataValue(BattlePetsReborn.plugin, (int) kiekis));
                    BattlePetsReborn.spawning.update(pet, (BattlePetsReborn) BattlePetsReborn.plugin);
                    break;
                case "defense":
                    pet.setMetadata("Defense", new FixedMetadataValue(BattlePetsReborn.plugin, (int) kiekis));
                    BattlePetsReborn.spawning.update(pet, (BattlePetsReborn) BattlePetsReborn.plugin);
                    break;
                case "strength":
                    pet.setMetadata("Strength", new FixedMetadataValue(BattlePetsReborn.plugin, (int) kiekis));
                    BattlePetsReborn.spawning.update(pet, (BattlePetsReborn) BattlePetsReborn.plugin);
                    break;
            }
        ...
        if (args[0].equalsIgnoreCase("cancel") && sender instanceof Player) {
            if (sender.hasPermission("battlepets.cancel")) {
                if (BattlePetsReborn.pets.containsKey(((Player) sender).getUniqueId())) {
                    BattlePetsReborn.spawning.setTarget(BattlePetsReborn.pets.get(((Player) sender).getUniqueId()), null);
                }
                return true;
            } else {
                sender.sendMessage(Language.getMessage("no_permission"));
        ...
        if (args[0].equalsIgnoreCase("menu") && sender instanceof Player) {
            if (sender.hasPermission("battlepets.menu")) {
                if (BattlePetsReborn.pets.containsKey(((Player) sender).getUniqueId())) {
                    BattlePetsReborn.openmenu((Player) sender, BattlePetsReborn.pets.get(((Player) sender).getUniqueId()));
                } else {
                    sender.sendMessage(Language.getMessage("NoPetAlive"));
                }
                return true;
        ...
        if (args[0].equalsIgnoreCase("name") && sender instanceof Player) {
            if (sender.hasPermission("battlepets.name")) {
                ...
                if (args.length == 2) {
                    ...
                    if (args[1].length() > BattlePetsReborn.namesize) {
                        sender.sendMessage(Language.getMessage("pet_name_toolong"));
                        return true;
                    }
                    LivingEntity pet = BattlePetsReborn.pets.get(((Player) sender).getUniqueId());
                    pet.setMetadata("Name", new FixedMetadataValue(BattlePetsReborn.plugin, args[1]));
        ...
        if (args[0].equalsIgnoreCase("egg")) {
            ...
            if (!BattlePetsReborn.statsai.containsKey(type) && !type.equalsIgnoreCase("random")) {
            ...
            if (type.equalsIgnoreCase("endermite"))
                type = "block";
            MobStats stats = BattlePetsReborn.statsai.get(type);
            ...
            sender.sendMessage(Language.getMessage("cmd_egg_added"));
                ...
                ItemStack item = BattlePetsReborn.createEgg(type, argumentai);
                ItemMeta meta = item.getItemMeta();
                if (args.length > 3) {
                    String name = "";
                    for (int i = 3; i < args.length - 1; i++) {
