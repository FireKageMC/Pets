...
import me.rokomilic.battlepetsreborn.Database;
import me.rokomilic.battlepetsreborn.Language;
import me.rokomilic.battlepetsreborn.versions.v1_12_2.Util1_12;
...
public class PlayerEvents implements Listener {
    ...
    public PlayerEvents(BattlePetsReborn plugin) {
        this.plugin = plugin;
        ...
        if (event.getRightClicked() instanceof Player) {
            ...
            if (battles.containsKey(p1.getUniqueId())) {
                if (battles.containsKey(p2.getUniqueId())) {
                    ...
                    if (battles.get(p2.getUniqueId()) == null || !battles.get(p2.getUniqueId()).equals(p1.getUniqueId())) {
                        ...
                        if (!BattlePetsReborn.pets.containsKey(p2.getUniqueId())) {
            ...
            if (battles.containsKey(p2.getUniqueId()))
                if (battles.get(p2.getUniqueId()).equals(p1.getUniqueId())) {
                    ...
                    //FIGHT.
                    if (BattlePetsReborn.pets.containsKey(p1.getUniqueId()) && BattlePetsReborn.pets.containsKey(p2.getUniqueId())) {
                        LivingEntity pet1, pet2;
                        pet1 = BattlePetsReborn.pets.get(p1.getUniqueId());
                        pet2 = BattlePetsReborn.pets.get(p2.getUniqueId());
                        BattlePetsReborn.spawning.setTarget(pet1, pet2);
                        BattlePetsReborn.spawning.setTarget(pet2, pet1);
    ...
    public void onDMGpl(EntityDamageByEntityEvent event) {
        ...
        if (p != null) {
            if (!BattlePetsReborn.pets.containsKey(p.getUniqueId())) return;
            LivingEntity pet = BattlePetsReborn.pets.get(p.getUniqueId());
            if (event.getEntity() == pet) {
                event.setCancelled(true);
                return;

            }
            BattlePetsReborn.spawning.setTarget(pet, (LivingEntity) event.getEntity());
        }
        if (event.getEntity() instanceof Player) {
            p = (Player) event.getEntity();
            if (BattlePetsReborn.pets.containsKey(p.getUniqueId())) {
                LivingEntity pet = BattlePetsReborn.pets.get(p.getUniqueId());
                ...
                if (attacker != null) BattlePetsReborn.spawning.setTarget(pet, attacker);
            }
        }
    }
    ...
    public void interact(PlayerInteractEvent event) {
        ...
        if (!BattlePetsReborn.AllWorlds && !BattlePetsReborn.worlds.contains(event.getPlayer().getWorld().getName())) {
            event.getPlayer().sendMessage(Language.getMessage("disabled_world"));
            return;
        }
        if (BattlePetsReborn.wg != null)
            if (!BattlePetsReborn.wg.isAllowed(event.getPlayer())) {
                event.getPlayer().sendMessage(Language.getMessage("disabled_zone"));
                return;
            }
        if (BattlePetsReborn.pets.containsKey(event.getPlayer().getUniqueId())) {
            event.getPlayer().sendMessage(Language.getMessage("second_pet"));
            return;
        }
        LivingEntity pet = BattlePetsReborn.spawning.SpawnCreature(event, plugin);
        if (pet != null)
            BattlePetsReborn.pets.put(event.getPlayer().getUniqueId(), pet);
    ...
    public void onQuit(PlayerQuitEvent event) {
        ...
        if (BattlePetsReborn.pets.containsKey(event.getPlayer().getUniqueId()) && Database.enabled) {
            LivingEntity pet = BattlePetsReborn.pets.get(event.getPlayer().getUniqueId());
            Database.SavePet(pet);
            BattlePetsReborn.spawning.returnPet(pet);
            pet.remove();
            BattlePetsReborn.pets.remove(event.getPlayer().getUniqueId());
        } else if (BattlePetsReborn.pets.containsKey(event.getPlayer().getUniqueId()))
            BattlePetsReborn.return_pet(event.getPlayer());
    }
    ...
    public void onDeath(PlayerDeathEvent event) {
        if (BattlePetsReborn.pets.containsKey(event.getEntity().getUniqueId()) && Database.enabled) {
            LivingEntity pet = BattlePetsReborn.pets.get(event.getEntity().getUniqueId());
            Database.SavePet(pet);
            BattlePetsReborn.spawning.returnPet(pet);
            pet.remove();
            BattlePetsReborn.pets.remove(event.getEntity().getUniqueId());
        } else if (BattlePetsReborn.pets.containsKey(event.getEntity().getUniqueId())) {
            ItemStack ite = BattlePetsReborn.return_pet(event.getEntity());
    ...
    public void onNoPVP(EntityDamageByEntityEvent event) {
        ...
        if (p == null) return;
        if (BattlePetsReborn.PVP) return;
        event.setCancelled(true);
    ...
    public void onChat(AsyncPlayerChatEvent event) {
        if (namechanging.contains(event.getPlayer().getUniqueId())) {
            if (BattlePetsReborn.pets.containsKey(event.getPlayer().getUniqueId())) {
                LivingEntity pet = BattlePetsReborn.pets.get(event.getPlayer().getUniqueId());
                if (event.getMessage().length() > BattlePetsReborn.namesize) {
                    event.getPlayer().sendMessage(Language.getMessage("pet_name_toolong"));
                    return;
                }
                pet.setMetadata("Name", new FixedMetadataValue(BattlePetsReborn.plugin, event.getMessage()));
    ...
    public void onCommand(PlayerCommandPreprocessEvent event) {
        ...
        cmd = cmd.substring(1);
        if (BattlePetsReborn.aliases.contains(cmd)) {
    ...
    public void onAnvil(InventoryClickEvent event) {
        if (event.getInventory() instanceof AnvilInventory) {
            ...
            if (BattlePetsReborn.version.equals("v1_12_R1")) {
                if (!Util1_12.fromItemStack(event.getCurrentItem()))
                    event.setCancelled(true);
