...
public class EntityEvents implements Listener {
    ...
    public EntityEvents(BattlePetsReborn plugin) {
        this.plugin = plugin;
    ...
    public void onDMGpet(EntityDamageByEntityEvent event) {
        ...
        if (!(event.getEntity() instanceof Damageable)) return;
            if (event.getEntity() instanceof ArmorStand)
                if (event.getEntity().hasMetadata("Owner")) {
                    event.setCancelled(true);
                    BattlePetsReborn.pets.get(UUID.fromString(event.getEntity().getMetadata("Owner").get(0).asString())).damage(event.getDamage(), event.getDamager());
                    return;
                }
        ...
        BattlePetsReborn.AddXP(pet, event.getDamage());
    }
    ...
    public void onDMGonPet(EntityDamageByEntityEvent event) {
        ...
        LivingEntity pet = null;
            if (event.getEntity() instanceof ArmorStand) {
                pet = BattlePetsReborn.pets.get(UUID.fromString(event.getEntity().getMetadata("Owner").get(0).asString()));
                event.setCancelled(true);
        ...
        if (typeconf.equalsIgnoreCase("endermite"))
            typeconf = "block";
        MobStats stats;
        stats = BattlePetsReborn.statsai.get(typeconf);
        ...
        if (event.getDamager() instanceof LivingEntity) {
            if (event.getDamager() instanceof Player) {
                ...
                if (!p.getUniqueId().equals(UUID.fromString(event.getEntity().getMetadata("Owner").get(0).asString())))
                    BattlePetsReborn.spawning.setTarget(pet, (LivingEntity) event.getDamager());
                return;
            }
            BattlePetsReborn.spawning.setTarget(pet, (LivingEntity) event.getDamager());
        }
    ...
    public void onDMG(EntityDamageByEntityEvent event) {
        if (event.getDamager() instanceof Egg) {
            event.getDamager().setMetadata("Cancel", new FixedMetadataValue(BattlePetsReborn.plugin, " "));
        }
    }
    ...
    public void onUnload(ChunkUnloadEvent event) {
    	for (Entity entity : event.getChunk().getEntities()) {
    		if (entity.hasMetadata("Owner")) {
                BattlePetsReborn.return_pet(Bukkit.getPlayer(UUID.fromString(entity.getMetadata("Owner").get(0).asString())));
    		}
    	}
    }
}
