...
public class Spawning_v1_12_R1 implements Spawning {
    ...
    public LivingEntity SpawnCreature(PlayerInteractEvent event, BattlePetsReborn plugin) {
        ...
        if (!BattlePetsReborn.statsai.containsKey(stats)) {
        ...
        MobStats statsai = BattlePetsReborn.statsai.get(stats);
        String mobtype;
        int level, points;
        double hp;
    ...
    public void update(LivingEntity pet, BattlePetsReborn plugin) {
        ...
        if (typeconf.equalsIgnoreCase("endermite"))
            typeconf = "block";
        MobStats statsai = BattlePetsReborn.statsai.get(typeconf);
    ...
    public void setTarget(LivingEntity pet, LivingEntity target) {
        if ((target instanceof Player) && !BattlePetsReborn.PVP) return;
