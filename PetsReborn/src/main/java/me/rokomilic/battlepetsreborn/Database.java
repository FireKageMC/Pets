...
public class Database {
    ...
    public static void Enable() {
        table = "pets";
        enabled = BattlePetsReborn.plugin.getConfig().getBoolean("MySQL.Enabled");
        if (!enabled) return;
        Connection con = null;
        Statement st = null;
        ResultSet rs = null;
        port = BattlePetsReborn.plugin.getConfig().getString("MySQL.Port");
        url = "jdbc:mysql://" + BattlePetsReborn.plugin.getConfig().getString("MySQL.IP") + ":" + port + "/" + BattlePetsReborn.plugin.getConfig().getString("MySQL.Database") + "?useUnicode=true&characterEncoding=utf-8";
        user = BattlePetsReborn.plugin.getConfig().getString("MySQL.Username");
        password = BattlePetsReborn.plugin.getConfig().getString("MySQL.Password");
        ...
        try {
            ...
            BattlePetsReborn.plugin.getLogger().info("Couldn't connect to MySQL.");
            enabled = false;
            return;
        }
    ...
    public static LivingEntity LoadPet(Player p) {
        if (!BattlePetsReborn.AllWorlds && !BattlePetsReborn.worlds.contains(p.getWorld().getName()))
            return null;
        ...
        MobStats stats = BattlePetsReborn.statsai.get(tipas);
        ...
        pet = BattlePetsReborn.spawning.SpawnCreature(new PlayerInteractEvent(p, Action.RIGHT_CLICK_BLOCK, item, p.getLocation().getBlock(), BlockFace.DOWN), (BattlePetsReborn) BattlePetsReborn.plugin);
        if (pet != null)
            BattlePetsReborn.pets.put(p.getUniqueId(), pet);
        return pet;
    }
}
